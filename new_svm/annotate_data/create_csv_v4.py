...
def get_code(method):
	try:
		iq_code = codeParser((method.replace('comments','code'))+'_code')
		code = iq_code.get_listOf_methodObj()
		return code[0].getInputTypes(), code[0].getOutput()
	except:
		return [], ''
...
def create_features(method_a, method_b, api):
	...
	#f.close()
	iq_in, iq_out = get_code(method_a)
	#print iq_in, iq_out	
	#iq_code = open((method_a.replace('comments','code'))+'_code','r').read()
	...
	for match in matches:
		features[match] = calc_feature_vector(match,iq_text,iq_in,iq_out)
	...
	# now do the correct one
 	features[method_b] = calc_feature_vector(method_b,iq_text,iq_in, iq_out)
...
def calc_feature_vector(match,iq_text,iq_in,iq_out):
	feature_vector = list()
	f = open(match)
	text = f.read().lower()
	f.close()
	text = get_first_line(match)
	m_in, m_out = get_code(match)
	#iq_nouns,iq_verbs = filter_pos(iq_text)
	...
	feature_vector.append(jaccard_dist(iq_in, m_in))
	if m_out == iq_out:
